...
- `--role realtime`：按 durable source state 轮询来源，执行已 admission 的 R0 Run，扫描到期 Forecast，并消费 committed outbox。
...
不创建第二套业务执行链、账本、队列、Agent loop、Gate 或发布权。来源仍只产出 `TextEnvelope`，worker 复用 Kernel admission、R0 LangGraph、R2-R bounded graph、确定性 Gate、账本和 outbox。Evolution/research worker 不得修改 source cursor、Gate、active pointer 或交易权限；DSH Session/JSONL 只作为运行轨迹。
