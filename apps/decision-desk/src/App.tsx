...
import { Activity, AlertTriangle, ArrowUpRight, Ban, BarChart3, Bell, BookOpen, BrainCircuit, CheckCircle2, ChevronRight, CircleHelp, Clock3, Database, FileText, History, Inbox, Layers3, Menu, Network, PanelLeftClose, RotateCcw, ScanSearch, Search, ServerCog, Settings2, ShieldCheck, Sparkles, Target, Undo2, X } from 'lucide-react'
...
import { EvolutionJobQueue, OperationsPage, PageTitle } from './OperationsPage'
...
const nav = [
  ...
  { label: 'Operations', icon: ServerCog },
...
function App() {
  ...
  const [workspaceView, setWorkspaceView] = useState('Research')
  ...
  const evolutionJobsQuery = useQuery({ queryKey: ['evolution-jobs'], queryFn: api.evolutionJobs, refetchInterval: 5_000, retry: false })
  const operationsQuery = useQuery({ queryKey: ['operations'], queryFn: api.operations, refetchInterval: 5_000, retry: false })
  const promotionMutation = useMutation({
    mutationFn: api.promotionDecision,
    onSuccess: () => { void queryClient.invalidateQueries({ queryKey: ['evolution-overview'] }); void queryClient.invalidateQueries({ queryKey: ['evolution-jobs'] }); void queryClient.invalidateQueries({ queryKey: ['operations'] }) },
  })
  const onlineServices = operationsQuery.data?.services.filter((item) => item.status === 'online').length ?? 0
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${collapsed ? 'collapsed' : ''}`}>
      ...
      <nav>{nav.map(({ label, icon: Icon }) => <button className={`nav-item ${workspaceView === label ? 'active' : ''}`} key={label} aria-label={label} title={collapsed ? label : undefined} onClick={() => setWorkspaceView(label)}><Icon size={17} />{!collapsed && <span>{label}</span>}{!collapsed && label === 'Inbox' && summary && <span className="nav-count">{summary.inbox.pending_count}</span>}</button>)}</nav>
      {!collapsed && <div className="sidebar-bottom"><button className="side-health" onClick={() => setWorkspaceView('Operations')}><span className={`live-dot ${onlineServices < 4 ? 'warning' : ''}`} /> <span>Services</span><span className="side-health-time">{onlineServices}/4</span></button><button className="nav-item"><Settings2 size={17} /><span>Settings</span></button><button className="nav-item"><CircleHelp size={17} /><span>Runbook</span></button></div>}
    </aside>
    <main className="main-content">
      <header className="topbar"><div className="breadcrumbs"><span>Decision Desk</span><ChevronRight size={14} /><strong>{workspaceView}</strong></div><div className="top-actions"><div className="search-shell"><Search size={15} /><input placeholder="Search events, runs, assets" aria-label="Search" /></div><button className="icon-button" aria-label="Notifications"><Bell size={17} /><span className="notification-dot" /></button><div className="owner-avatar">C</div></div></header>
      {workspaceView === 'Research' ? <ResearchPage operations={operationsQuery.data} operationsLoading={operationsQuery.isLoading} operationsFailed={operationsQuery.isError} /> : <>
      {workspaceView === 'Operations' ? <OperationsPage data={operationsQuery.data} loading={operationsQuery.isLoading} failed={operationsQuery.isError} /> : workspaceView === 'Evolution' ? <div className="content-wrap evolution-page"><PageTitle title="Evolution" eyebrow="OWNER-GATED EVOLUTION" copy="从已验证结果与失败模式生成候选，经 replay、holdout、shadow 后只进入 owner review。" trailing={operationsQuery.data ? `${operationsQuery.data.jobs.pending_owner_review} 项待审查` : undefined} /><div className="evolution-layout"><EvolutionJobQueue jobs={evolutionJobsQuery.data} loading={evolutionJobsQuery.isLoading} failed={evolutionJobsQuery.isError} /><section className="panel evolution-assets-panel"><div className="panel-header"><div><h2>候选、评测与晋级</h2><p>Active pointer 只能由 owner command 改变</p></div></div>{evolutionQuery.isLoading && <div className="workspace-state"><Activity size={17} />正在读取演进资产…</div>}{evolutionQuery.isError && <div className="workspace-state danger"><AlertTriangle size={17} />演进资产不可用，未使用演示数据。</div>}{!evolutionQuery.isLoading && !evolutionQuery.isError && <div className="evolution-panel-body"><EvolutionPanel evolution={evolutionQuery.data ?? null} deciding={promotionMutation.isPending} onReview={(candidateId, evaluationRefs) => api.promotionReview(DOMAIN_PACK_REF, { candidate_id: candidateId, evaluation_refs: evaluationRefs })} onDecision={(payload) => promotionMutation.mutateAsync({ ...payload, domain_pack_ref: DOMAIN_PACK_REF })} /></div>}</section></div></div> : <div className="content-wrap">{!summary && <><PageTitle title="Inbox" eyebrow="LIVE DECISION SYSTEM" copy="事件、证据和待确认动作集中在这里。系统不会把未经 Gate 的候选当成结论。" /><div className={`workspace-state ${summaryQuery.isError ? 'danger' : ''}`}>{summaryQuery.isError ? <AlertTriangle size={17} /> : <Activity size={17} />}{summaryQuery.isError ? 'Inbox 数据不可用，未使用演示数据替代。' : '正在读取业务账本…'}</div></>}{summary && <><section className="page-heading"><div><div className="eyebrow"><span className="live-dot" />LIVE DECISION SYSTEM</div><h1>Inbox</h1><p>事件、证据和待确认动作集中在这里。系统不会把未经 Gate 的候选当成结论。</p></div><div className="heading-actions"><button className="button secondary" onClick={() => setSubmitOpen(true)}><FileText size={15} />提交文本</button><button className="button primary" onClick={() => setSubmitOpen(true)}><Sparkles size={15} />新建分析</button></div></section><section className="metrics-grid"><Metric icon={Inbox} label="待处理事件" value={summary.inbox.pending_count} hint="需要人工确认" tone="amber" /><Metric icon={Activity} label="运行中" value={summary.inbox.running_count} hint="自动刷新 · 5s" tone="blue" /><Metric icon={Target} label="近 30 天预测" value={summary.forecast_count} hint={`${summary.evaluated_count} 已有结果`} tone="green" /><Metric icon={Database} label="当前策略" value={summary.active_strategy} hint={summary.active_pack} tone="purple" /></section><section className="workspace-grid"><div className="panel inbox-panel"><div className="panel-header"><div><h2>Recent events</h2><p>按接收时间排序 · Point-in-time evidence</p></div></div><div className="table-wrap"><table><thead><tr><th>事件</th><th>状态</th><th>策略</th><th>延迟</th><th>接收时间</th><th><span className="sr-only">操作</span></th></tr></thead><tbody>{summary.inbox.latest.map((run) => <tr key={run.run_id} onClick={() => setSelected(run)}><td><div className="event-cell"><span className="event-type"><BarChart3 size={15} /></span><div><strong>{run.headline ?? '分析运行中'}</strong><span>{run.event_id}</span></div></div></td><td><StatusPill status={run.gate_status} runStatus={run.status} errorCode={run.error_code} /></td><td><span className="mono">{run.strategy_version}</span></td><td><span className="mono">{run.latency_ms ? `${(run.latency_ms / 1000).toFixed(1)}s` : '—'}</span></td><td><span className="time">{formatRelative(run.updated_at)}</span></td><td><ChevronRight size={16} className="row-chevron" /></td></tr>)}</tbody></table>{summary.inbox.latest.length === 0 && <div className="workspace-state"><Inbox size={17} />暂无真实事件；可提交文本或启用来源。</div>}</div></div><div className="side-column"><div className="panel health-panel"><div className="panel-header"><div><h2>System health</h2><p>来源、运行和资产状态</p></div><span className="health-badge"><span className="live-dot" />{health?.status ?? summary.health_status}</span></div><div className="health-list"><HealthRow icon={Activity} label="Decision Core" value={health?.status === 'degraded' ? 'Degraded' : 'Healthy'} detail={`${health?.running_runs ?? summary.inbox.running_count} running · ${health?.failed_runs ?? 0} failed`} tone={health?.status === 'degraded' ? 'warning' : 'success'} /><HealthRow icon={Network} label="Evidence sources" value={health ? `${sourceHealth.healthyCount}/${health.sources.length} healthy` : 'Unavailable'} detail={sourceHealth.detail} tone={sourceHealth.degraded.length || !health?.sources.length ? 'warning' : 'success'} /><HealthRow icon={BrainCircuit} label="Agent runtime" value={operationsQuery.data?.runtime.mode ?? 'Unavailable'} detail={operationsQuery.data ? `${operationsQuery.data.runtime.runtime_id} · canary ${operationsQuery.data.runtime.live_canary_status}` : 'Operations API unavailable'} tone={operationsQuery.data?.runtime.mode === 'provider' ? 'success' : 'warning'} /><HealthRow icon={Database} label="Ledger" value="SQLite WAL" detail="cursor, runs and outbox durable" tone="success" /></div><button className="panel-link" onClick={() => setWorkspaceView('Operations')}>Open operations <ArrowUpRight size={14} /></button></div><div className="panel principle-panel"><div className="principle-icon"><ShieldCheck size={18} /></div><div><h3>发布由 Gate 决定</h3><p>Agent 只能提交候选。证据、反方、时间一致性和操作字段通过代码检查后，结果才会进入发布账本。</p></div></div></div></section><section className="bottom-grid"><div className="panel signal-panel"><div className="panel-header"><div><h2>Forecast coverage</h2><p>输出窗口和结果闭环</p></div></div><div className="coverage-row"><div className="coverage-score"><span>{summary.evaluated_count}</span><small>/ {summary.forecast_count}</small><strong>已评估</strong></div><div className="coverage-bars"><CoverageBar label="已产生预测" value={summary.forecast_count ? 100 : 0} tone="blue" /><CoverageBar label="已获得结果" value={summary.forecast_count ? Math.round((summary.evaluated_count / summary.forecast_count) * 100) : 0} tone="green" /><CoverageBar label="待补标签" value={summary.forecast_count ? Math.max(0, Math.round(((summary.forecast_count - summary.evaluated_count) / summary.forecast_count) * 100)) : 0} tone="amber" /></div></div></div><div className="panel assets-panel"><div className="panel-header"><div><h2>Personal assets</h2><p>从结果中沉淀的方法，而不是 Prompt 集合</p></div></div><div className="asset-items"><div className="asset-empty"><Layers3 size={17} /><div><strong>{summary.evaluated_count ? '资产提炼待审核' : '暂无已验证资产'}</strong><span>{summary.evaluated_count ? `${summary.evaluated_count} 个结果可进入 Experience 提炼流程` : '完成 Forecast Outcome 后，经验才会进入资产候选。'}</span></div></div></div></div></section></>}</div>}
      </>}
    </main>
    ...
    {!['Inbox', 'Operations', 'Evolution'].includes(workspaceView) && <WorkbenchDrawer view={workspaceView} memos={memosQuery.data ?? []} evolution={evolutionQuery.data ?? null} loading={memosQuery.isLoading || evolutionQuery.isLoading} failed={memosQuery.isError || evolutionQuery.isError} deciding={promotionMutation.isPending} onReview={(candidateId, evaluationRefs) => api.promotionReview(DOMAIN_PACK_REF, { candidate_id: candidateId, evaluation_refs: evaluationRefs })} onDecision={(payload) => promotionMutation.mutateAsync({ ...payload, domain_pack_ref: DOMAIN_PACK_REF })} onClose={() => setWorkspaceView('Inbox')} />}
